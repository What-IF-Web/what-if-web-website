...
scriptsMap.forEach((scriptInfo, selector)=>{
    ...
    if (scriptInfo && scriptInfo.src && scriptInfo.id) {
        ...
        if (element) {
            console.log(`Found element for selector: ${selector}, loading script: ${scriptInfo.src}`);
            preloadAndExecuteScript(scriptInfo.src, scriptInfo.id);
        } else console.log(`No element found for selector: ${selector}, skipping script: ${scriptInfo.src}`);
    } else console.log(`Invalid scriptInfo for selector: ${selector}`, scriptInfo);
});
