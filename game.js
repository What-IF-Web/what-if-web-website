...
window.startGame = function() {
    ...
    scene("battle", ()=>{
        ...
        const boss = add([
            rect(BOSS_HITBOX_W, BOSS_HITBOX_H),
            color(255, 0, 0),
            area({
                width: BOSS_HITBOX_W,
        ...
        const trashCountUI = add([
            text(`Trash Cleared: 0 / ${TRASH_SPAWN_LIMIT}`),
            pos(12, 32),
            fixed(),
            {
                value: 0
        ...
        function updateTrashCountUI() {
            trashCountUI.text = `Trash Cleared: ${trashClearedCount} / ${TRASH_SPAWN_LIMIT}`;
        }
        const healthbar = add([
            rect(width(), 24),
            pos(0, 0),
    ...
    try {
        ...
        console.error("Failed to start initial game scene.", e);
        add([
            text("Error loading game. Check console.", {
                size: 20,
                width: width() - 40
            }),
            pos(width() / 2, height() / 2),
